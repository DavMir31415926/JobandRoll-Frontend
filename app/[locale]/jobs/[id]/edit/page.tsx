  ...
  useEffect(() => {
    const fetchJob = async () => {
      try {
        ...
        // Set location for display - parse the stored location format
        if (job.location) {
          // If location is stored as "Hallwang, 05 (AT)", parse it properly
          const locationParts = job.location.match(/^(.+?),?\s*(\d+)?\s*\(([A-Z]{2})\)$/);
          if (locationParts) {
            setSelectedLocation({ 
              name: job.location,
              admin_level1: locationParts[2] || null,
              country: locationParts[3] || 'AT'
            });
          } else {
            // Fallback for simpler location formats
            setSelectedLocation({ 
              name: job.location,
              admin_level1: null,
              country: 'AT'
